...
func Validate(params interface{}) error {
	...
	// Try to cast to Validator interface first
	if validator, ok := params.(Validator); ok {
		return validator.Validate()
	}

	// Fall back to reflection for compatibility
	return validateWithReflection(params)
}

// Validator interface for high-performance validation
type Validator interface {
	Validate() error
}

// validateWithReflection is the fallback method using reflection for compatibility
func validateWithReflection(params interface{}) error {
	if params == nil {
		return nil
	}
...
// High-performance validation methods using type assertions
// These methods should be used instead of reflection-based validation
