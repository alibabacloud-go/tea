...
func TestWebSocketReconnectWhenAlreadyConnected(t *testing.T) {
	...
	res, err := newWebSocketClientAndConnect(request, runtimeObject)
	if err != nil {
		t.Fatalf("Initial connection failed: %v", err)
	}

	// Verify client is connected
	if !res.WebSocketClient.IsConnected() {
		t.Fatal("Client should be connected")
	}

	// Try to reconnect while already connected
	response, err := res.WebSocketClient.Reconnect()
